...
public class PrintPic {
    ...
    public void initPaint() {
        this.paint = new Paint();// Create a new one brush
    ...
    public byte[] printDraw() {
        ...
        imgbuf[0] = 29;// Hexadecimal0x1D
        imgbuf[1] = 118;// Hexadecimal0x76
        imgbuf[2] = 48;// 30
        imgbuf[3] = 0;// Bitmap mode 0,1,2,3
        // Represents the number of bytes bitmap in the horizontal xL+xH × 256）
        ...
        s = 7;
        for (int i = 0; i < getLength(); i++) {// Height of cyclic bitmap
            for (int k = 0; k < this.width / 8; k++) {// Weight of cyclic bitmap
                int c0 = nbm.getPixel(k * 8 + 0, i);// Returns the color of the specified coordinate
                int p0;
